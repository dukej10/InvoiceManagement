package co.com.credit.dynamodb.helper;

import co.com.credit.dynamodb.DynamoDBTemplateAdapter;
...
class TemplateAdapterOperationsTest {
    ...
    @Mock
    private DynamoDbAsyncTable<ClientEntity> customerTable;

    private ClientEntity modelEntity;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.openMocks(this);

        when(dynamoDbEnhancedAsyncClient.table("table_name", TableSchema.fromBean(ClientEntity.class)))
                .thenReturn(customerTable);

        modelEntity = new ClientEntity();
    ...
    void modelEntityPropertiesMustNotBeNull() {
        ClientEntity modelEntityUnderTest = new ClientEntity("id", "atr1");
    ...
    void testSave() {
        ...
        when(mapper.map(modelEntity, ClientEntity.class)).thenReturn(modelEntity);
